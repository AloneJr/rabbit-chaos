...
class Coelho {
    ...
    setCoordenada(coord) { this.#coordenada = coord; }
...
class Jogo {
    ...
    instanciarCoelhos() {
        let cont = 0;
        while (cont < 5) { // 5 coeios para começar
    ...
    iniciarRodada() {
        for (let coelho of this.#coelhos) {
            ...
            while (!moveu && tentativas < 10) { // Previne loop infinito se ficar encurralado
                ...
                if (mov.novoI >= 0 && mov.novoI < this.#tamanho && 
                    mov.novoJ >= 0 && mov.novoJ < this.#tamanho) {
